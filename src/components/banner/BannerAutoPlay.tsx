/* Create by hoangchung from Macbook Pro 2020 20/05/2021 */
React.StrictMode;
...
export default class BannerAutoPlay extends React.Component<
  BannerPros,
  BannerState
> {
  public static defaultProps: BannerPros = {
  ...
  componentDidUpdate(prevProps: BannerPros) {
  ...
  // public componentWillReceiveProps(nextProps: BannerPros) {
  ...
  private gotoPage(index: number, animated: boolean = true, cb = () => {}) {
    ...
    if (animated) {
      this.pageAnimation = this.props.animation!!(
    ...
    if (childrenNum < 1) {
      ...
      content = (
        <Animated.View
          ...
          {pages.length == 0 ?<View><Text>Nothing News</Text></View>:(pages)}
        </Animated.View>
      );
    }
